...
const require_index = require('./components/realtime/subscribe/index.cjs');
...
const useRealtime = ({ channel, topics, token: tokenInput, key, enabled = true, bufferInterval = 0, validate = true, apiBaseUrl, historyLimit = 100, reconnect = true, reconnectMinMs = 250, reconnectMaxMs = 5e3, pauseOnHidden = true, autoCloseOnTerminal = true }) => {
	...
	const tokenInputKey = typeof tokenInput === "function" ? "factory" : tokenInput ? isSubscriptionToken(tokenInput) ? JSON.stringify({
		apiBaseUrl: tokenInput.apiBaseUrl,
		channel: typeof tokenInput.channel === "string" ? tokenInput.channel : tokenInput.channel.name,
		key: tokenInput.key,
	...
	const [allMessages, setAllMessages] = (0, react.useState)([]);
	const [messageDelta, setMessageDelta] = (0, react.useState)([]);
	const [messagesByTopic, setMessagesByTopic] = (0, react.useState)({});
	const [lastMessage, setLastMessage] = (0, react.useState)(null);
	const [error, setError] = (0, react.useState)(null);
	const [connectionStatus, setConnectionStatus] = (0, react.useState)("idle");
	const [pauseReason, setPauseReason] = (0, react.useState)(null);
	const [runStatus, setRunStatus] = (0, react.useState)("unknown");
	const [result, setResult] = (0, react.useState)(void 0);
	const [isVisible, setIsVisible] = (0, react.useState)(() => isDocumentVisible());
	const subscriptionRef = (0, react.useRef)(null);
	const readerRef = (0, react.useRef)(null);
	const messageBufferRef = (0, react.useRef)([]);
	const bufferIntervalRef = (0, react.useRef)(bufferInterval);
	const messageLimitRef = (0, react.useRef)(historyLimit);
	const runStatusRef = (0, react.useRef)(runStatus);
	const tokenInputRef = (0, react.useRef)(tokenInput);
	(0, react.useEffect)(() => {
	...
	(0, react.useEffect)(() => {
		...
		const resolveToken = async () => {
			...
			if (currentTokenInput && typeof currentTokenInput !== "function") {
				if (isSubscriptionToken(currentTokenInput)) return currentTokenInput;
				if (!channel || !topics) throw new Error("useRealtime token object did not include channel/topics and channel/topics were not provided");
				return {
					channel,
					topics,
		...
		const run = async () => {
			...
			while (!cancelled) {
				try {
					...
					if (cancelled) break;
					const stream = await require_index.subscribe({
						...token,
						validate,
...
//#endregion
exports.getClientSubscriptionToken = require_index.getClientSubscriptionToken;
