...
const require_triggers = require('./triggers.cjs');

//#region src/components/triggers/utils.ts
var EventValidationError = class EventValidationError extends require_NonRetriableError.NonRetriableError {
	constructor(message) {
		super(message);
...
function parseTrigger(trigger) {
	if (trigger instanceof require_triggers.EventType) return {
		eventName: trigger.name,
		schema: trigger.schema
	};
	if (trigger.event instanceof require_triggers.EventType) return {
	...
	if (trigger.cron) return {
		eventName: require_consts.internalEvents.ScheduledTimer,
		schema: null
	};
...
function createValidatorsByTrigger(schemasByTrigger) {
	...
	if (!(validatorsByTrigger[require_consts.internalEvents.FunctionInvoked] !== void 0)) {
		const allSchemas = Object.values(schemasByTrigger).flat();
		validatorsByTrigger[require_consts.internalEvents.FunctionInvoked] = createSchemaUnionValidator(allSchemas);
	}
