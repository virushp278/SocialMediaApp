...
class RuntimeNodeInstrumentation extends instrumentation_1.InstrumentationBase {
    ...
    constructor(config = {}) {
        ...
        this._collectors = [
            new eventLoopUtilizationCollector_1.EventLoopUtilizationCollector(this._config),
            new eventLoopTimeCollector_1.EventLoopTimeCollector(this._config),
            new eventLoopDelayCollector_1.EventLoopDelayCollector(this._config),
            new gcCollector_1.GCCollector(this._config),
            new heapSpacesSizeAndUsedCollector_1.HeapSpacesSizeAndUsedCollector(this._config),
        ];
    ...
    _unregisterExceptionHandlers() {
        if (this._onUncaughtExceptionHandler) {
            process.removeListener('uncaughtExceptionMonitor', this._onUncaughtExceptionHandler);
            this._onUncaughtExceptionHandler = undefined;
        }
    }
    _handleUncaughtException(error, _origin) {
        this._emitExceptionLog(error, api_logs_1.SeverityNumber.FATAL, 'uncaughtException');
    }
    _emitExceptionLog(error, severityNumber, eventType) {
        if (!this.isEnabled()) {
            return;
        }
        const config = this.getConfig();
        ...
        if (config.applyCustomExceptionAttributes) {
            try {
                ...
                this._diag.error('applyCustomExceptionAttributes threw while handling an exception', err);
            }
        }
        const timestamp = (0, core_1.hrTime)();
        const errorLog = {
            body: 'exception',
            exception: error,
            severityNumber,
    ...
    _forceFlushLogs() {
        ...
        try {
            void loggerProvider.forceFlush().catch(err => {
                this._diag.error('loggerProvider.forceFlush failed while handling an exception', err);
            });
        }
        catch (err) {
            this._diag.error('loggerProvider.forceFlush threw while handling an exception', err);
        }
    }
}
